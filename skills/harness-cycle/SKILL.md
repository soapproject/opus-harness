...
1. 規模分流：預估 ≤ 2 片且小改 → ephemeral（`tmp/plan-<cycle_id>.md`，wrapup 時刪）；否則 persistent（`plans/YYYY-MM-DD-<topic>.md`，wrapup 時搬 `plans/done/`）。
2. 委派 writing-plans。硬規則：每片 ≤ 一個行為（約 ≤ 3 檔案）；每片驗收 = 具體指令＋預期結果。
...
- subagent-driven 模式：每片派 fresh subagent，輸入 = 計畫該片全文（不依賴對話記憶）。
最小 checklist（降級用）：嚴格紅綠重構循環＋每片 commit，自行維護 state 欄位。

## Phase 4 review

adversarial-review §C（必要時 §D）。修正循環中的程式碼改動回到 Phase 3 紀律（測試先行）。
