...
**受保護集合**＝`config.git.protectedBranches` ∪ {存在的 `main`/`master`} ∪ {可解析的 `origin/HEAD`}；config 缺 `git` 節 → 視同全集（保守向）。集合內＝發布用分支；**其餘一切分支＝開發/hotfix 類**（白名單法，不靠命名猜測）。

- **受保護分支**：agent 不得以任何方式推進或破壞其 ref（merge／push／fast-forward／reset／branch -f／改名／刪除）。唯一途徑＝開 PR 附證據（scorecard、驗收查核表），停等人類核准。這是常設閘門（constraints.md human-merge-gate），不是升級提問——不用 AskUserQuestion 等回覆，開完 PR 即最終回報；發生於 wrapup 之後，與 stop-gate、升級白名單無碰撞。
- **行為由 cycle 啟動分支推導**：啟動於受保護分支 → 切短命 feature 分支開發，cycle 末 PR 回啟動分支；啟動於開發/hotfix 類分支 → 該分支即整合目標，**主動分段 merge**。`config.git.integrationBranch` 僅為顯式 override，不得屬受保護集合——衝突時視同未設定並升級告知。
- **分段 merge（開發/hotfix 類）**：每完成一個可獨立站立的階段就 merge 回整合目標，不囤到 cycle 末。前置條件（機械錨點）：`red_count`==0、HEAD==`last_green_commit`、該片 verifier 已過。手法：`git merge --no-ff --no-commit` → 有衝突就解（屬程式碼工作，回 TDD 紀律）→ 跑 config 全套測試 → 綠才 commit merge，否則 `git merge --abort`。棘輪的 stash／reset --hard 只動 feature 分支，**永不改寫整合分支**。
- **Phase 4 時序**：整合分支＝小組審查前的 WIP 整合層；panel 抓到 blocker → fix-forward 以新階段 merge 修復（不改寫歷史）；受保護分支的 PR 只在 panel 過後開。整合分支含未經人類核准的變更——從其 checkout 執行 `claude plugin update` 仍屬 cutover 禁令範圍（README）。
- **merge message 寫 why 不寫 what，且自含**：格式 `merge(<topic>): <一句 why>`＋body 3–6 行寫「解什麼問題／為何此作法／捨棄了什麼」。what 由 diff 自證。**不引用任何檔案路徑**（ephemeral 工件會消失、persistent 工件會搬家）；後續 agent 靠 `git log --merges` 即可追蹤決策鏈。
- **細節超出 message 容量 → 臨時 CLAUDE.md 傳遞**：寫 `.claude/harness/tmp/merge-<topic>.md`（背景／取捨／風險／驗證證據），作為 cycle 內交給 subagent 的傳遞媒介（在 subagent prompt 中指名要讀它）；屬 tmp 工件不入版控、**永不被持久工件引用**，wrapup 時要點蒸餾回計畫 `## Decisions` 後刪除。
