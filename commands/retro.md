...
「這個任務存成 benchmark case？」同意 → 在 opus-harness repo（向使用者確認本機 clone 的路徑）的 `bench/cases/case-NN-<topic>/` 建：
- `spec.md`：複製本次量化 spec
- `rubric.md`：從 templates/rubric.md 複製，填入本 case 特化要點與 baseline 總分
- `baseline/`：`git diff <state.start_commit>..HEAD` 存為 `baseline.diff`＋最終 scorecard 分數存 `scores.json`
