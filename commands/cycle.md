...
4. `plans/` 有未完成的 persistent 計畫（檔內仍有 `- [ ]`）？→ 問使用者：恢復它（state 指回該計畫、phase=executing）或歸檔（搬 `plans/done/`）後開新的。不默默開新的。同時檢查 `.claude/harness/tmp/plan-*.md` 與 `.claude/harness/tmp/merge-*.md` 殘留（上次 cycle 異常中斷遺留）→ 刪除並在回報中提及。
