BeforeAll {
  $script:gate = Join-Path $PSScriptRoot ".." "hooks" "gate-ratchet.ps1"
  function New-Fixture {
    param([string]$StateJson, [string]$ConfigJson, [string]$Root)
    if (-not $Root) { $Root = Join-Path ([IO.Path]::GetTempPath()) ("oh-ratchet-" + [guid]::NewGuid()) }
...
AfterAll {
  Get-ChildItem ([IO.Path]::GetTempPath()) -Filter "oh-ratchet-*" -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
  Get-ChildItem ([IO.Path]::GetTempPath()) -Filter "oh-rat-*" -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
  ...
  $docsRoot = Join-Path ([IO.Path]::GetTempPath()) "oh-rat-docs"
...
Describe "gate-ratchet" {
  It "red_count below limit exits 0" {
    ...
    $target = Join-Path $f "src" "a.ts"
    $payload = '{"cwd":"' + ($f -replace '\\','\\') + '","tool_name":"Edit","tool_input":{"file_path":"' + ($target -replace '\\','\\') + '"}}'
  ...
  It "harness path is whitelisted exits 0" {
    $f = New-Fixture `
      -StateJson '{"phase":"executing","suspended":false,"red_count":2,"active_slice":"S3","last_green_commit":"abc1234"}' `
      -ConfigJson '{"commands":{"test":"exit 0"},"voting":{"ratchetLimit":2}}'
    $target = Join-Path $f ".claude" "harness" "state.json"
    $payload = '{"cwd":"' + ($f -replace '\\','\\') + '","tool_name":"Edit","tool_input":{"file_path":"' + ($target -replace '\\','\\') + '"}}'
    $r = Invoke-Gate -Fixture $f -PayloadJson $payload
    $r.Code | Should -Be 0
  }
  ...
  It "docs path is whitelisted exits 0" {
    $f = New-Fixture `
      -StateJson '{"phase":"executing","suspended":false,"red_count":2,"active_slice":"S3","last_green_commit":"abc1234"}' `
      -ConfigJson '{"commands":{"test":"exit 0"},"voting":{"ratchetLimit":2}}'
    $target = Join-Path $f "docs" "plan.md"
    $payload = '{"cwd":"' + ($f -replace '\\','\\') + '","tool_name":"Edit","tool_input":{"file_path":"' + ($target -replace '\\','\\') + '"}}'
    $r = Invoke-Gate -Fixture $f -PayloadJson $payload
    $r.Code | Should -Be 0
  }
  ...
  It "no harness dir exits 0" {
    $tempCwd = [IO.Path]::GetTempPath()
    $target = Join-Path $tempCwd "src" "a.ts"
    $payload = '{"cwd":"' + ($tempCwd -replace '\\','\\') + '","tool_name":"Edit","tool_input":{"file_path":"' + ($target -replace '\\','\\') + '"}}'
  ...
  It "notebook_path target with red=2 exits 2" {
    $f = New-Fixture `
      -StateJson '{"phase":"executing","suspended":false,"red_count":2,"active_slice":"S3","last_green_commit":"abc1234"}' `
      -ConfigJson '{"commands":{"test":"exit 0"},"voting":{"ratchetLimit":2}}'
    $target = Join-Path $f "notebooks" "analysis.ipynb"
    $payload = '{"cwd":"' + ($f -replace '\\','\\') + '","tool_name":"NotebookEdit","tool_input":{"notebook_path":"' + ($target -replace '\\','\\') + '"}}'
    $r = Invoke-Gate -Fixture $f -PayloadJson $payload
    $r.Code | Should -Be 2
  }
  ...
  It "harness-edit-allowed audit: red=2 edit harness file → exit 0 and telemetry event=harness-edit-allowed" {
    $f = New-Fixture `
      -StateJson '{"phase":"executing","suspended":false,"red_count":2,"active_slice":"S3","last_green_commit":"abc1234"}' `
      -ConfigJson '{"commands":{"test":"exit 0"},"voting":{"ratchetLimit":2}}'
    $harnessFile = Join-Path $f ".claude" "harness" "state.json"
    $payload = '{"cwd":"' + ($f -replace '\\','\\') + '","tool_name":"Edit","tool_input":{"file_path":"' + ($harnessFile -replace '\\','\\') + '"}}'
    $r = Invoke-Gate -Fixture $f -PayloadJson $payload
    $r.Code | Should -Be 0
    $telemetryPath = Join-Path $f ".claude" "harness" "telemetry.jsonl"
    Test-Path $telemetryPath | Should -BeTrue
