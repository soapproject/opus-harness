BeforeAll {
  $script:runner = "$PSScriptRoot\..\bench\runner.ps1"
  $script:repo = Join-Path $env:TEMP ("oh-bench-repo-" + [guid]::NewGuid())
  ...
  $script:casesDir = Join-Path $env:TEMP ("oh-bench-cases-" + [guid]::NewGuid())
  ...
  $script:resultsDir = Join-Path $env:TEMP ("oh-bench-results-" + [guid]::NewGuid())

  # Create stub agent scripts to avoid inner-quote stripping when passed via -File
  $script:stubsDir = Join-Path $env:TEMP ("oh-bench-stubs-" + [guid]::NewGuid())
...
Describe "bench runner" {
  ...
  It "stub agent creates file - verify passes and runner exits 0" {
    ...
    $agentCmd = "powershell -NoProfile -File $stubPath"
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $script:runner `
